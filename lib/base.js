...
var Base = module.exports = function Base(args, options) {
  ...
  this._hooks = [];
  this._composedWith = [];
  // this._composedWith is dependent upon the 'run' method.  This new array will hold composed
  //   generators without having any dependency with them being ran.
  this._composedGenerators = [];
  this._transformStreams = [];
  
  // In the case that this generator was composed from another, _parentGenerator will reference it.
  this._parentGenerator = null;
...
Base.prototype.run = function run(cb) {
  ...
  // Due to separate memFs stores per composed generator, we now only need to schedule the
  //   conflicts stage for the root generator
  if (this._parentGenerator === null) {
    
    // We need to perform the transforms per composed-generator, then merge all the mem-fs
    //   stores into this one prior to checking for conflicts and committing to the fs.
    this.env.runLoop.add('conflicts', function preFileCommit(done) {
        performTransforms(this, function() {
            depthFirstStoreMerge(this);
            done();
        }.bind(this));
    }.bind(this));
    ...
    this.fs.store.on('change', writeFiles);
    writeFiles();
...
Base.prototype.composeWith = function composeWith(namespace, options, settings) {
  ...
  generator._parentGenerator = this;
  this._composedGenerators.push(generator);
  if (this._running) {
    generator.run();
  } else {
...
Base.prototype._writeFiles = function (done) {
  ...
  this.fs.commit([conflictChecker], function() {
      done();
  });
};
...
function performTransforms(aGenerator, done) {
  if (aGenerator._transformStreams.length) {
    var stream = aGenerator._transformStreams.reduce(function(stream, filter) {
      return stream.pipe(filter);
    }, aGenerator.fs.store.stream());
    var calls = [];
    calls.push(function(cb) {
      stream.on('finish', cb);
    });

    if (aGenerator._composedGenerators.length) {
      aGenerator._composedGenerators.forEach(function(composedGenerator) {
        calls.push(function(cb) {
          performTransforms(composedGenerator, cb);
        });
      });
    }
...
function depthFirstStoreMerge(aGenerator) {
  var resStore;

  if (aGenerator._composedGenerators.length) {
    var composedStores = [];
    aGenerator._composedGenerators.forEach(function(composedGenerator) {
      composedStores.push(depthFirstStoreMerge(composedGenerator));
    });
    aGenerator.fs.store.merge(composedStores);
  }

  return aGenerator.fs.store;
}
